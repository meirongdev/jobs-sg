...
- **状态口径（2026-08-07 收敛）**：一轮只要没能完整记录它抓到的东西，就记 `partial`，不记 `success`。触发条件包括页请求失败、熔断、对账偏差 ≥2%、**归档写入失败、upsert 失败、归档 Close（gzip flush）失败**。
  ...
  > **偏差 ≥2% 仍记 partial，但（2026-08-10 起）不再 `errors++`**：它是「本轮扫描不足以支撑*按缺席*关闭」的判断，不是故障。计进 `errors` 会流进 `jobs_sg_ingest_errors_total`，而那条告警的语义是「MCF 字段形状变了」—— 等于让一个谨慎但健康的夜晚和一个真坏了的夜晚长得一模一样。partial 本身已经足够：它扣住 `last_success`，`JobsSgReconcileStale` 照常盯得到。
...
遍历 /v2/jobs 全量（total≈86,678 → ~867 页 @limit=100，1.5s 间隔 ≈ 22 分钟）
  → 收集全部在架 uuid 集合 S；抓取条数与 API total 偏差 ≥2% 或触发熔断 → status='partial'
     -- total 取**最后一页**的读数，不取全程最大值（2026-08-10 修）。
     -- 扫描要 25 分钟，total 在底下一直动；旧实现锁 max(page.Total)，
     -- 相当于拿一个峰值去除一个累加和，量纲就不对，任何瞬时抬升都被永久固化。
     -- 2026-08-09 唯一一次对账因此读出 4.5%（85487 vs 早已回落的 89531），
     -- 而那轮其实已经翻到了最后一页。
  ...
  → 关闭判定（v2.1：仅当本轮扫描干净才执行，partial 扫描严禁批量关闭）：
     ...
     -- 仍然整块跳过；但**只是偏差 ≥2%** 时，仅停掉「按缺席关闭」那一支：
