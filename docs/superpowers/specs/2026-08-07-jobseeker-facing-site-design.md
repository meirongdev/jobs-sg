...
- **薪资透明率**：`salary_hidden=0 AND salary_min IS NOT NULL` 的占比，整体 + 按 company_type 拆分。
  - **透明率的分子与中位数的样本不是同一个集合，不得复用同一个谓词。** 中位数只能取月薪岗位（`salary_type='Monthly'` 且 min/max 齐全），因为跨薪资单位求中位数没有意义；而透明率回答的是「这个岗位肯不肞告诉我薪资」，与计价单位无关——年薪、时薪岗位公开了薪资就是透明的。用中位数的谓词当透明率分子，会把非月薪的公开薪资岗算成「不透明」，在一个分母为全部 SWE 岗位的比率里**系统性低估透明率**。
  - 因此实现上必须是两个谓词：`disclosedSalary`（月薪 + min/max 齐全，仅供中位数/分位数/溢价）与一个独立的透明率谓词（本行定义）。两处样本量各自披露，不要混用。
...
- **能力边界：竞争度只有快照，没有时间序列。** `MarkSeen` 每次采集**覆盖** `view_count`/`application_count`（`internal/store/jobs.go`），不保留历史，所以「本周新增投递」「竞争度逐周变化」这类问题本系统答不了——日均归一化已是从单点快照能榨出的极限。若日后要做竞争度趋势，需要新增按采集日快照这两个计数的表；在那之前，页面不得暗示这些数字有时间维度。
