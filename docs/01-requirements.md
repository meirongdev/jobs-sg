...
| 路由 | 回答的问题 | 渲染 | 状态 |
|---|---|---|---|
| `/` | 现在热不热：在架量、周新增、WoW、12 周趋势、方向/资历/工作模式/雇佣类型分布、入门岗绝对数 | 现算，60s 缓存 | 已上线 |
| `/tech` | 学什么最划算：需求排名、动量、薪资溢价、入门友好度、MCF 原生技能标签 | 现算，60s 缓存 | 已上线 |
| `/pay` | 我值多少：分位数网格、经验阶梯、薪资透明率 | 现算，60s 缓存 | 已上线 |
| `/companies` | 投谁：持续招聘者、类型分布、各家竞争度与透明率、岗位寿命、幽灵岗信号 | 现算，60s 缓存 | 已上线 |
...
- **每个对外页面必须标注数据延迟**（日批采集，最长 24h），不得让读者误读为实时职位源。这条是对外化之后新增的红线：把陈旧数据呈现成实时职位源，会让读者去投一个已经关掉的岗位。
  - 实现：各页页脚固定一句延迟声明（`internal/view` 的 market/tech/pay/companies/notice 模板与 `internal/report/render.go` 的周报页脚）。周报另加一句说明它是所覆盖那一周的快照。
