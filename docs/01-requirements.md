...
| 路由 | 回答的问题 | 渲染 | 状态 |
|---|---|---|---|
| `/` | 现在热不热：在架量、周新增、WoW、方向/资历分布 | 现算，60s 缓存 | **计划（A-2b）**；当前仍托管最新周报，尚无周报时渲染说明页 |
...
| `/w/{YYYY-Www}` | 历史周报（不可变归档） | 静态，`report` CronJob 产出 | 已上线 |
...
- **每个对外页面必须标注数据延迟**（日批采集，最长 24h），不得让读者误读为实时职位源。这条是对外化之后新增的红线：把陈旧数据呈现成实时职位源，会让读者去投一个已经关掉的岗位。
  - 实现：各页页脚固定一句延迟声明（`internal/view` 的 tech/pay/notice 模板与 `internal/report/render.go` 的周报页脚）。周报另加一句说明它是所覆盖那一周的快照。
